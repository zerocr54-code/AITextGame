...
public class GameController : ControllerBase
{
    ...
    public async Task<IActionResult> StartGame([FromBody] GameSetup setup)
    {
        ...
        var systemPrompt = $$"""
            ...
            Oyuncunun Adı: {{(string.IsNullOrWhiteSpace(setup.KarakterAdi) ? "Henüz bilinmiyor. İlk uygun NPC doğal biçimde adını sorsun; oyuncu cevap verene kadar isim uydurma." : setup.KarakterAdi)}}
            Oyunun Teması: {{setup.Tema}}
            Ek Detaylar: {{setup.Detay}}

            KURALLAR:
            1. Hikâyeyi sakin bir uyanış, uzun çevre tanıtımı veya sıradan bekleyişle BAŞLATMA. Oyuncu ilk cümleden itibaren seçilen tema ve dünyaya uygun, hâlihazırda devam eden önemli bir olayın ortasında olsun.
            1.1. Macera/fantastik temada takip, alarm, saldırı tehdidi, büyü kazası, kayıp kişi veya acil görev gibi aktif bir olay kullan.
            1.2. Günlük hayat, ilişki, zenginlik veya sosyal temada beklenmedik karşılaşma, önemli haber, aile/iş krizi, yanlış anlaşılma, davet, yarışma veya tartışma gibi aktif fakat temaya uygun bir olay kullan.
            1.3. Açılış olayı oyuncuyu seçim yapmaya zorlasın fakat oyuncunun ne yaptığını onun adına yazma.
            ...
            7. İlk sahnenin action değeri "idle" olmasın. Her cevabın sonunda aşağıdaki biçimde geçerli JSON sahne verisi üret. JSON dışında etiket içine açıklama yazma:
            [SCENE_JSON]
            {"worldStyle":"fantasy|isekai|medieval|modern|cyberpunk|post_apocalyptic|historical|space","environment":"forest|fantasy_village|castle|tower|room|road|mountain|dungeon|mansion|cafe|school|modern_city|coast","location":"hikâyede geçen gerçek mekân adı","setting":"indoor|outdoor","time":"day|sunset|night","weather":"clear|rain|snow|fog","mood":"calm|happy|tense|mysterious|sad","action":"idle|talk|walk|run|sit|celebrate|argue|fight|give|look","actionText":"Sahnede olan ana hareketi 2-5 kelimeyle yaz","player":{"name":"Oyuncunun bilinen adı veya Sen","x":25,"emotion":"neutral|happy|sad|angry|surprised","outfit":"casual|formal|royal|armor|robe|uniform","hair":"black|brown|blonde|red|gray","color":"#22c55e","speech":"Oyuncu konuştuysa çok kısa sözü"},"npcs":[{"name":"NPC veya yaratık adı","kind":"human|dragon|animal|monster","x":70,"emotion":"neutral","outfit":"casual","color":"#3b82f6","speech":"Varsa çok kısa sözü"}],"objects":["table","chair"],"caption":"Ana olayı açıkça anlatan tek kısa cümle"}
            [/SCENE_JSON]
            8. worldStyle oyun evrenine, environment ise hikâyede anlatılan gerçek mekâna kesinlikle uymalı. Fantastik/isekai/orta çağ dünyasında modern_city kullanma.
            9. O anda metinde görünen veya konuşan tüm önemli NPC ve yaratıkları npcs listesine koy. Uzakta görülen ejderha gibi ana olayları atlama.
            10. x değerleri 15 ile 85 arasında olmalı. En fazla 3 NPC ve 5 nesne üret. Karakterlerin renk ve görünüşlerini önceki sahnelerle tutarlı koru.
            """;
    ...
    private async Task<ApiResult> AskGemini(List<object> history)
    {
        ...
        try
        {
            ...
            var models = new[] { "gemini-2.5-flash", "gemini-2.5-flash-lite" };
            foreach (var model in models)
            {
                var url = $"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={Uri.EscapeDataString(key.Trim())}";
                ...
                if (!response.IsSuccessStatusCode)
                    return new(false, null, $"Gemini isteği başarısız: {(int)response.StatusCode}", (int)response.StatusCode);

                using var doc = JsonDocument.Parse(body);
                var text = doc.RootElement.GetProperty("candidates")[0].GetProperty("content").GetProperty("parts")[0].GetProperty("text").GetString();
                return string.IsNullOrWhiteSpace(text)
                    ? new(false, null, "Gemini boş yanıt verdi.", 502)
                    : new(true, text, null, 200);
            }

            return new(false, null, "Gemini ücretsiz kullanım sınırına ulaşıldı. Bir süre bekleyip yeniden dene.", 429);
        }
